...
// Middleware for validating the auth header
...
// Middleware for validating the GeoJSON linestring
const validateLinestring = (req, res, next) => {
  const linestring = req.body.linestring;


  // Example: Check if the linestring has a minimum of 2 points
...
app.post('/intersect', authMiddleware, validateLinestring, (req, res) => {
  ...
  for (const line of lines) {
    ...
    // Convert line coordinates to LineString geometry
    const lineGeom = lineString(line.line.coordinates);

    // Check if the linestring intersects with each line
    for (const otherLine of lines) {
      if (line.id !== otherLine.id) {
        // Convert otherLine coordinates to LineString geometry
        const otherLineGeom = lineString(otherLine.line.coordinates);

        const intersection = turf.lineIntersect(lineGeom, otherLineGeom);

        if (intersection.features.length > 0) {
          intersecting.push({
            id: otherLine.id,
            intersectionPoints: intersection.features.map((feature) =>
